            ...
            {section.items.map((item) => (
              <li key={item.to}>
                <Link
                  ...
                  className="tap-target flex items-center gap-3 rounded-xl px-3 py-2.5 text-[0.95rem] text-sidebar-foreground transition-colors hover:bg-sidebar-accent data-[status=active]:bg-sidebar-accent data-[status=active]:font-semibold data-[status=active]:text-sidebar-accent-foreground"
                >
  ...
  return (
    <div className="flex min-h-dvh-safe bg-background">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 border-b bg-background/85 pt-safe backdrop-blur-xl lg:hidden">
          <div className="flex h-14 items-center gap-2 px-2 pl-safe pr-safe">
            <Sheet open={open} onOpenChange={setOpen}>
              ...
              <SheetContent side="left" className="w-[19rem] gap-0 bg-sidebar p-0 pt-safe pb-safe">
        ...
        <main className="min-w-0 flex-1 pl-safe pr-safe">{children}</main>
      </div>
    </div>
  );
}
